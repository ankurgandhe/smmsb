function [gammahat, B, lastll, phi, inphi] = sMMSB_Estep(y, alpha, B, gammahat, maxit, eta, sigma2, labelVec)
% y: N*N
% alpha: 1*K
% B: K*K
% gammahat: N*K
...
while true
    ...
    for i=1:N
        ...
        %plabel = eta'*eta; %For multinormial ,  
		plabel = plabel(:);
        delta =reshape(e2(:,i)*e3,[K*K,N])...
            .* (Blin*y(i,:)+nBlin*(1-y(i,:))) .* repmat(exp(plabel),1,N) ;  % delta_u_v : size K^2*N
        ...
        newgamma = newgamma + (reshape(sum(reshape(delta,[K,K,N]),1),[K,N]))'... 
            .* repmat(rev_sum_delta,[1,K]);     % newgamma(j,:) %+ sum_delta_v'; %
