%%%%% SUPERVISED MMSB
function [alpha,betahat,gammahat,eta,sigma2] = MMSB_EM(adjMatrix,a,K,labelVec,filename,maxit)
...
dlmwrite('../s-gamma.txt',gammahat);
dlmwrite('../s-alpha.txt',alpha);
betahat = ones(1,K) - 1e-1;% rand(1,K);
%etalabel = [1 20 40 60];
eta = 1/K *ones(1,K); %./max(labelVec) ; %ones(1,K);
